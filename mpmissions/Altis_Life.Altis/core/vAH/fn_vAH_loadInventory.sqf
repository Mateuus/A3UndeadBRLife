...
_price = 0;
{if(_x select 5 isEqualTo _uid && _x select 1 isEqualTo 0) then {playerInventoryArray pushBack [_x select 3 select 0,_x select 2,_x select 7,"ymenu",_x select 8,_x select 4,_x select 0]};} forEach all_ah_items; //GET Y MENU AND THEIR STATUS
{if(_x select 5 isEqualTo _uid && _x select 1 isEqualTo 1) then {playerInventoryArray pushBack [_x select 3 select 0,_x select 2,_x select 7,"imenu",_x select 8,_x select 4,_x select 0]};} forEach all_ah_items; //GET I MENU AND THEIR STATUS
	...
	} else {
{
	...
	switch (_type) do {
		case "imenu": {
			...
			_time = round (_time / 3);
			if (_time <= 0) then {_price = 0; if (_status isEqualTo 1) then {_toDel = round(_time + 216); _timeDisplay = format[": Removed in %1 Hrs",_toDel]} else {_timeDisplay = "";};} else {_timeDisplay = format[": Expires in %1 Hrs",_time]};
			if (_price isEqualTo 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Auction Price $%1",[_price]call life_fnc_numberText];};
			};
		case "ymenu": {
