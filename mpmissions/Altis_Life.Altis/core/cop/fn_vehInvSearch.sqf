...
*/
private["_illegalPrice","_vehicle","_vehicleInfo","_value","_list"];
...
_illegalValue = 0;
{
	...
	_illegalPriceSearch = M_CONFIG(getNumber,"VirtualItems",_var,"sellPrice");
	
	if(_isIllegalItem isEqualTo 1 ) then{
	
	if(_illegalPriceSearch == -1) then{
		_illegalPrice = M_CONFIG(getNumber,"VirtualItems",_var,"buyPrice");
	}else{
		...
		if(!isNull (missionConfigFile >> "VirtualItems" >> _var >> "processedItem")) then {
			...
			if(_illegalPriceSearch == -1) then{
			   _illegalPrice = M_CONFIG(getNumber,"VirtualItems",_illegalItemProcessed,"buyPrice");
			}else{
