			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == localize "str_channel_side") then
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						...
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED! This is your first warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						...
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED! This is your second warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_two = true;
						systemChat ("Please do not use voice on sidechat, this is your third warning..");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED! This is your third warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 3) then {
					if (isNil "warn_four") then {
						warn_two = true;
						systemChat ("Please do not use voice on sidechat, this is your fourth warning..");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED! This is your fourth warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 4) then {
					if (isNil "warn_five") then {
						warn_three = true;
						systemChat ("PLEASE DO NOT USE VOICE ON SIDECHAT!! This is your fifth warning!");
						systemChat ("You will be frozen and then disconnected!");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED! This is your fifth warning!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 5) then {
					if (isNil "warn_five") then {
						...
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON SIDE CHAT TURN IT OFF NOW OR BE KILLED OR DISCONNECTED!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me >= 6) then {
					if (isNil "warn_last") then {
						...
						startLoadingScreen ["You are being disconnected, dork"];
