...
if(EQUAL(LIFE_SETTINGS(getNumber,"player_moneyLog"),1)) then {
	if(EQUAL(LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging"),1)) then {
		money_log = format ["withdrew %1 from their gang bank. Gang Bank Balance: %2  Bank Balance: %3  On Hand Balance: %4",_value,[_gFund] call life_fnc_numberText,[TTPBANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
	} else {
		money_log = format ["%1 - %2 withdrew %3 from their gang bank. Gang Bank Balance: %4  Bank Balance: %5  On Hand Balance: %6",profileName,(getPlayerUID player),_value,[_gFund] call life_fnc_numberText,[TTPBANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
	};
